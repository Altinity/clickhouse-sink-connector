...
| [`config_checksum.yml`](sink-connector-lightweight/deployment/config_checksum.yml) | Deployment config with all tunable parameters |
...
```sql
SELECT offset_val FROM altinity_sink_connector.replica_source_info FINAL LIMIT 1
```
...
Add to the `checksum:` section of [`config_checksum.yml`](sink-connector-lightweight/deployment/config_checksum.yml):
...
The following block shows all new keys to add to the `checksum:` section of [`config_checksum.yml`](sink-connector-lightweight/deployment/config_checksum.yml), with their recommended defaults and available options:
