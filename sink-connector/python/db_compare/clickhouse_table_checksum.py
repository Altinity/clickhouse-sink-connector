...
def filter_excluded_columns(columns_metadata, excluded_columns):
    """Drops the connector-generated columns from a table's column metadata.

    A column is kept, despite being named in ``excluded_columns``, when the
    same name prefixed with an underscore is also a column of the table: that
    means the SOURCE declares this column and the connector's generated one was
    renamed out of its way, so the source column is real data and must stay in
    the comparison.

    :param columns_metadata: rows of (name, type, is_nullable, numeric_scale),
        in table order.
    :param excluded_columns: column names to drop.
    :return: the rows to include in the checksum, in the order given.
    """
    ...
    return filtered_columns_metadata
...
def get_table_checksum_query(conn, table):
    ...
    filtered_columns_metadata = filter_excluded_columns(columns_metadata, excluded_columns)
...
def main():
  ...
  Compute the table checksum using the same technique as pt-checksum, md5 algorithm.
    ...
    # _row_key is generated by the connector for a KEYLESS source table (no
    # PRIMARY KEY, no UNIQUE key) to give ReplacingMergeTree a row identity. It
    # exists only in ClickHouse, so like _version and is_deleted it must be
    # excluded or the two sides hash different column sets and every keyless
    # table is reported as a mismatch. The prefixed variants cover the case
    # where the source table already declares a column called _row_key and the
    # generated one was renamed out of the way; the "don't exclude X when _X is
    # also present" rule above then keeps the SOURCE column in the comparison.
    parser.add_argument('--exclude_columns', help='columns exclude', nargs='*',
                        default=['_sign,_version,is_deleted,_is_deleted,'
