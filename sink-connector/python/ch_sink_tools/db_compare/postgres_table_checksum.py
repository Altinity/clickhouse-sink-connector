...
def compute_chunk_checksum(pg_host, pg_user, pg_password, pg_port, pg_database,
                            ...
                            debug_out_file=None, snapshot_conn=None):
    """
    Execute a single chunk checksum query.

    When snapshot_conn is provided (snapshot_mode=True), executes on that
    shared REPEATABLE READ connection instead of opening a new one.
    The caller is responsible for the connection lifecycle; this function
    will NOT close snapshot_conn.

    When snapshot_conn is None (legacy mode), opens its own connection
    and closes it after the query — original behavior preserved.

    Returns (cnt, a, b, c, d) tuple or writes debug rows to file.
    """
    own_conn = (snapshot_conn is None)
    if own_conn:
...
def get_postgres_table_checksum(conn, table_name, columns_meta, pk_columns,
    ...
    snapshot_conn   : optional shared REPEATABLE READ psycopg2 connection
                      ...
                      When None, each chunk opens its own connection (legacy).
    ...
    # Divide into chunks (uses conn for MIN/MAX pk query)
    ...
    if snapshot_conn is not None:
        ...
        for chunk in chunks:
            ...
            logging.debug(f"Chunk SQL (snapshot) for {schema}.{table_name}: {sql[:200]}...")

            result = compute_chunk_checksum(
                pg_host, pg_user, pg_password, pg_port, pg_database,
                table_name, schema, sql, debug_output, debug_limit,
                debug_out_file, snapshot_conn=snapshot_conn,
            )
            if result is not None:
                chunk_results.append(result)
    else:
        # Legacy mode: each chunk runs in its own thread with its own connection.
        futures_list = []
        with concurrent.futures.ThreadPoolExecutor(max_workers=threads_per_table) as executor:
            for chunk in chunks:
                ...
                fut = executor.submit(
                    ...
                    pg_host, pg_user, pg_password, pg_port, pg_database,
                    table_name, schema, sql, debug_output, debug_limit,
                    debug_out_file,  # snapshot_conn=None (default)
                )
