...
public class ClickHouseAutoCreateTableTest extends com.altinity.clickhouse.sink.connector.db.operations.ClickHouseAutoCreateTableBase {
    ...
    @Test
    public void testCreateTableSyntaxQuotesReservedAndSpecialIdentifiers() {
        ArrayList<String> primaryKeys = new ArrayList<>();
        primaryKeys.add("customerName");

        ClickHouseAutoCreateTable act = new ClickHouseAutoCreateTable();

        String query = act.createTableSyntax(primaryKeys, "order-details", "default",
                createFields(), this.columnToDataTypesMap, false, false, null, config);

        Assert.assertTrue(query.startsWith("CREATE TABLE `default`.`order-details`("));
    }
