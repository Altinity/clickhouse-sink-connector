...
public class DdlReplayIdempotencyTest {
    ...
    /**
     * CHANGE COLUMN is translated as MODIFY (old name) plus RENAME (old to
     * new). Once the rename has been applied the old name is gone, so on
     * replay the MODIFY half fails with Code: 10 unless it is guarded. This
     * is the CHANGE-specific exception to
     * {@link #testModifyColumnIsDeliberatelyNotGuarded()}: the guard is on
     * the synthesised statement, not on a user MODIFY.
     */
    @Test
    public void testChangeColumnModifyHalfIsGuarded() {
        String q = parse("alter table t change column old_name new_name int", "t");

        Assert.assertTrue("the MODIFY half of a CHANGE must carry IF EXISTS, or a replayed "
                + "CHANGE fails with Code: 10 once the rename has happened: " + q,
                q.contains("modify column if exists old_name"));
        Assert.assertTrue("the RENAME half must stay guarded: " + q,
                q.contains("rename column if exists old_name to new_name"));
    }

    /**
     * A replayed unguarded CREATE TABLE ... LIKE fails with Code: 57
     * TABLE_ALREADY_EXISTS, exactly like the column CREATE that is already
     * guarded.
     */
    @Test
    public void testCreateTableLikeIsGuarded() {
        String q = parse("create table new_tbl like orig_tbl", "new_tbl");

        Assert.assertTrue("CREATE TABLE ... LIKE must carry IF NOT EXISTS: " + q,
                q.contains("create table if not exists"));
    }

    /**
     * A rename is not self-idempotent: once applied the old table is gone, so
     * a replay of RENAME TABLE (either form) must be a no-op rather than a
     * stream-stalling failure. RENAME TABLE IF EXISTS is accepted by
     * ClickHouse 24.8 and applies to every clause of the statement.
     */
    @Test
    public void testRenameTableIsGuarded() {
        String q = parse("rename table a to b", "a");
        Assert.assertTrue("RENAME TABLE must carry IF EXISTS: " + q,
                q.contains("rename table if exists"));

        String alterForm = parse("alter table a rename to b", "a");
        Assert.assertTrue("ALTER TABLE ... RENAME TO must carry IF EXISTS: " + alterForm,
                alterForm.contains("rename table if exists"));
    }

    /**
     * MySQL binlogs a server-generated drop as
     * {@code DROP TABLE db.t /* generated by server *&#47;} -- WITHOUT the
     * IF EXISTS the user may have typed -- so the guard cannot be copied from
     * the source statement; it must always be emitted.
     */
    @Test
    public void testDropTableIsAlwaysGuarded() {
        // DESTRUCTIVE: none -- string assertion on generated DDL text; no
        // database is contacted and nothing is dropped.
        String q = parse("drop table t /* generated by server */", "t");

        // DESTRUCTIVE: none -- substring check on a generated string.
        Assert.assertTrue("DROP TABLE must always carry IF EXISTS, even when the source "
                + "statement did not: " + q,
                q.contains("drop table if exists"));
    }
