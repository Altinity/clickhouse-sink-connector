...
- Verification: the GTID same-millisecond tie across an insert-batch boundary (§3.1.1) is not yet covered by an automated test (gap). History-mode version collisions under `gtid_mode=OFF` (§3.5 b) and the `_operation` spelling mismatch (§3.5 c) are recorded gaps without a test.
