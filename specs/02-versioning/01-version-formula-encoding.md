...
**GTID text formats.** `source.gtid` is `uuid:n` for a classic MySQL GTID and `uuid:tag:n` for a tagged GTID (MySQL 8.3+). `setAdditionalMetaData` today parses the transaction number only from the two-segment form (`split(":").length == 2`); a tagged GTID leaves `gtid` unset, so the record silently takes path 2 and ranks in the sequence domain while its neighbours rank in the snowflake domain (**gap, tracked**; spec 02.06 §3.2.1). A value with no colon (MariaDB `domain-server-seq`) also leaves `gtid` unset.
...
- `ClickHouseStructTest.testGtid()` — the classic `uuid:n` form is parsed.
...
- Verification: the tagged-GTID form (§3.1) is not yet covered by an automated test (gap). The GTID same-millisecond tie across an insert-batch boundary (§3.1.1) is not yet covered by an automated test (gap). History mode (§3.5 a–d) is not yet covered by an automated test (gap).
