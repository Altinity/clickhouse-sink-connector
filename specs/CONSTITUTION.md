...
- `Replication.DdlTranslation`: ALTER clause classification for Specs 06.03/06.04/06.05/06.07/06.09 — no bare `ALTER TABLE`, an all-no-op statement is skipped, a change of the row identity (`ADD`/`DROP PRIMARY KEY`) or a widening/rename of a key column translates to a rebuild carrying exactly the representable clauses (a loud clause elsewhere in the statement still wins), every ADD COLUMN is preserved.
- `Replication.PkRebuild`: the primary-key rebuild of Spec 06.09 — re-keying the FINAL live rows by an injective map preserves each live row once with its version (`rebuild_view_eq`), copies no tombstone and keeps the live count (`rebuild_preserves_live_rows`).
