...
- `Replication.DdlTranslation`: ALTER clause classification for Specs 06.03/06.04/06.05/06.07/06.09 — no bare `ALTER TABLE`, an all-no-op statement is skipped, a widening key-column change is loud, a change of the row identity (`ADD`/`DROP PRIMARY KEY`) translates to a rebuild carrying exactly the representable clauses, every ADD COLUMN is preserved.
