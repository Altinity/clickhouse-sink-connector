...
translator path (`MySqlDDLParserListenerImpl`, replayed `CREATE TABLE`) — and
records where the two paths differ. The row-level write protocol is spec
...
- **Record path**: `sink-connector/src/main/java/com/altinity/clickhouse/sink/connector/db/operations/ClickHouseAutoCreateTable.java` — `createTableSyntax` (history columns, engine, `PARTITION BY`, `ORDER BY`, `TTL`).
- **DDL path**: `sink-connector-lightweight/src/main/java/com/altinity/clickhouse/debezium/embedded/ddl/parser/MySqlDDLParserListenerImpl.java` — `enterColumnCreateTable` (history columns with `DEFAULT`, `PARTITION BY`, `ORDER BY`, `TTL`), `isConnectorColumn` (excludes the history columns from identity checks).
...
Appended after the source columns, in this order, on both paths:

| Column | Record path (`ClickHouseAutoCreateTable`) | DDL path (`MySqlDDLParserListenerImpl`) |
|---|---|---|
| `_valid_from` | `DateTime` (no default) | `DateTime[('<tz>')] DEFAULT '2100-01-01 00:00:00'` |
| `_valid_to` | `DateTime DEFAULT '2100-01-01 00:00:00'` | `DateTime[('<tz>')] DEFAULT '2100-01-01 00:00:00'` |
| `_operation` | `LowCardinality(String)` | `LowCardinality(String)` |
| `is_deleted` | `UInt8` (emitted here, then **not** repeated with the engine columns) | `UInt8` (emitted with the engine columns, as in standard mode) |
| `_version` | `UInt64` | `UInt64` |
...
2. `_valid_from` has a `DEFAULT` on the DDL path only (**difference D-1**).
   The connector always binds `_valid_from` explicitly (12.03 §3.1), so the
   default is only observable for rows inserted by something other than the
   connector.
3. The DDL path adds the `DateTime` timezone suffix when
   `clickhouse.datetime.timezone` is configured
   (`DataTypeConverter.addTimeZoneToDateTimeType`); the record path emits a
   bare `DateTime` (**difference D-2**).
...
| `ORDER BY` | `(k1,...,kn,`_valid_to`)` where `k1..kn` is the standard-mode key (declared PK, else keyless all-columns fallback, spec 08.05 §3.2) | `(k1,...,kn,`_valid_to`)` on the normal branch; **`_valid_to` is NOT appended** when a schema-override `primary_key` is configured or when the `name(N)` regex clean-up branch fires (**Gap G-12.02-3**) |
| `PRIMARY KEY` | `(k1,...,kn)` when a declared key exists (without `_valid_to`) | not emitted separately |
| `TTL` | `` `_valid_to` + toIntervalDay(30) `` — **hardcoded 30**, ignores `replication.history.ttl` (**difference D-3**) | `` `_valid_to` + toIntervalDay(<replication.history.ttl>) `` |
...
- **Sorting-key rule of 06.05 §3.6 / 08.05 §3.2** (`ORDER BY tuple()` never
  emitted) applies unchanged; history mode only appends `_valid_to`.
...
- **Coverage gap**: no test pins the record-path DDL string (differences D-1..D-3 are unguarded), the override-`primary_key` branch of Gap G-12.02-3, or the timezone mismatch of Gap G-12.02-1.

---

## 6. Differences between the two creation paths and gaps (not fixed here)
| Id | Record path | DDL path | Consequence |
|---|---|---|---|
| D-1 | `_valid_from DateTime` | `_valid_from ... DEFAULT sentinel` | rows written by anything but the connector get `1970-01-01` vs the sentinel |
| D-2 | bare `DateTime` | `DateTime('<tz>')` when `clickhouse.datetime.timezone` is set | same table gets a different column type depending on which path created it (08.05 §3.1.1 rule violated) |
| D-3 | `TTL ... toIntervalDay(30)` hardcoded | `TTL ... toIntervalDay(replication.history.ttl)` | retention of an auto-created table ignores the configured TTL |
| G-12.02-1 | both | both | open-row predicate depends on the statement timezone equalling the column timezone; untested |
| G-12.02-2 | legacy sign engine + history columns | n/a | untested combination |
| G-12.02-3 | n/a | `_valid_to` not appended to `ORDER BY` on the `primary_key`-override and regex branches | the table is not an SCD2 table: every version of a key collapses under `ReplacingMergeTree` to the highest `_version` |
