...
5.  Set `snapshot.mode` to `initial` if you like to replicate existing records, set `snapshot.mode` to `no_data` to replicate schema and only the records that are modified after the connector is started.
