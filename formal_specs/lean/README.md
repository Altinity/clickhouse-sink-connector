...
| `clockSeed` / `below_clock_seed` | a start with no mark raises the floor to `clock + headroom`; a version `ts * M + c` with `ts ≤ clock`, `c < headroom * M` lies below that slot | The seed of a first start is a function of the clock alone — no scan of the targets (Invariant I14, spec 10.06). |
| `clock_restart_boundary` / `clock_seed_example` | every first delivery of a clock-seeded run exceeds every pre-restart version below the seeded slot; executed on a row five hours behind the clock | The restart fix holds on the first start after an upgrade without reading a single target table. |
